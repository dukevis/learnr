...
Import both shapefile layers.  View the extents of both layers using bbox() function.  If need be, use the spTransform function to project one layer to the other layer.  Create new object and set the newly projected layer to the polygon.  In order to export the new spatial data frame to a shapefile, use writeOGR command.  **Code is in the "Clip.R" script and the result will look like this:
